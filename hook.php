...
function work(string $pareName, $chat_id, $api) {
    ...
    if($moving == "sell")// пытаемся продать монету
    {
        ...
        {
            $quantity = $balances[substr($pareName, 0, -4)]["available"];// получаем количество монет для продажи
            sendMessage($chat_id, "__________");// сообщение о неудачной продаже
        ...
        else
        {
            sendMessage(
               $chat_id,
               $pareName." failed sell, now price: ".$price." < wanted price: ". $old_price*$max_multiplier
               );// сообщение о неудачной продаже
        }
    }
    else// пытаемся купить монету 
    {
        if($price < $min_price*$min_multiplier && $balances["USDT"]["available"] >= 12)// если цена ниже желаемой и на балансе есть минимальная сумма для покупк
        {
            $quantity = round(12/$price, $accuracy);// получаем количество монет для покупки
            
            sendMessage($chat_id, "__________");// сообщение об успешной покупке
            ...
            $order = $api->marketBuy($pareName, $quantity);// покупаем
        ...
        else
        {
            sendMessage(
              $chat_id,
              $pareName." failed buy, now price: ".$price." > wanted price: ". $min_price*$min_multiplier." balance ".$balances["USDT"]["available"]." minim ".$min_price. " saved minim ".$saved_min_price
              );// сообщение о неудачной покупке
        }
    }
}